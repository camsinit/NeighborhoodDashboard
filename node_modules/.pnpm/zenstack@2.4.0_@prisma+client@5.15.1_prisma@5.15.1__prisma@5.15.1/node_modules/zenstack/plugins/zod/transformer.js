...
class Transformer {
    constructor(params) {
        ...
        this.sourceFiles = [];
        this.delegateCreateUpdateInputRegex = /(\S+)(Unchecked)?(Create|Update).*Input/;
    ...
    generateObjectSchemaFields(generateUnchecked) {
        ...
        if (createUpdateMatch) {
            ...
            const dataModel = this.zmodel.declarations.find((d) => (0, ast_1.isDataModel)(d) && d.name === modelName);
            if (dataModel) {
                const discriminatorFields = dataModel.fields.filter(sdk_1.isDiscriminatorField);
