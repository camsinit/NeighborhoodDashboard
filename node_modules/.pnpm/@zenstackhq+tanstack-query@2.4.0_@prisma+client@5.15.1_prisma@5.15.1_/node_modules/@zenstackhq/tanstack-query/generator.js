...
function generateQueryHook(target, version, sf, model, operation, returnArray, optionalInput, overrideReturnType, overrideInputType, overrideTypeParameters, supportInfinite = false, supportOptimistic = false) {
    ...
    for (const generateMode of generateModes) {
        ...
        func.addStatements([
            ...
            `return use${generateMode}ModelQuery<TQueryFnData, TData, TError>('${model}', \`\${endpoint}/${(0, lower_case_first_1.lowerCaseFirst)(model)}/${operation}\`, args, options, fetch);`,
        ]);
    }
}
        ...
        declarations: [
            {
                name: `_mutation`,
                initializer: `
                    useModelMutation<${argsType}, DefaultError, ${overrideReturnType !== null && overrideReturnType !== void 0 ? overrideReturnType : model}, ${checkReadBack}>('${model}', '${httpVerb.toUpperCase()}', \`\${endpoint}/${(0, lower_case_first_1.lowerCaseFirst)(model)}/${operation}\`, metadata, options, fetch, ${checkReadBack})
                `,
            },
        ],
    });
...
function generateCheckHook(target, version, sf, model, prismaImport) {
    ...
    func.addStatements([
        makeGetContext(target),
        `return useModelQuery<boolean, boolean, TError>('${model.name}', \`\${endpoint}/${(0, lower_case_first_1.lowerCaseFirst)(model.name)}/check\`, args, options, fetch);`,
    ]);
}
